...
4. **标识符**：字母或 `_` 开头，后跟字母 / 数字 / `_`；**大小写敏感**：表名 / 列名按原拼写逐字符精确匹配（引用时必须与建表时一致，`student` 与 `StuDenT` 视为不同名字）。
...
> 名字解析规则：标识符（表名 / 列名）**大小写敏感**，按原拼写精确匹配；
> 关键字大小写不敏感（§1.2.3 / §5）。`CREATE TABLE student` 之后，
> 引用 `StuDenT` 视为另一张不存在的表（报 `UnknownTable`）。
